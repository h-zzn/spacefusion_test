...
public class StartVor : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (initialCharactersIn == false && arrangeHousesObj.GetComponent<Arrange>().characterInitialized == true)
        {
        ...
        // sharedspacePoints
        SharedSpacePoints sharedspacePoints = arrangeHousesObj.GetComponent<Arrange>().sharedspacePoints;
        ...
        // update user pos 
        if (arrangeHousesObj.GetComponent<Arrange>().characterInitialized == true)
        {
        ...
        //{
        //    feedOutpoints(item, exteriorVec4, interiorVec4, userPosVec4, housesCount, mainUser);
        //}
        ...
        for (int housesCount = 0; housesCount < houses.Count; housesCount++)
        {
            ...
            if (chooseHouseNum == housesCount)
            {
                ...
                GameObject[] boxes = GameObject.FindGameObjectsWithTag("house" + housesCount.ToString());

                foreach (var item in boxes)
                {
                    Renderer boxRenderer = item.GetComponent<Renderer>();
                    boxRenderer.material.SetInt("_isBox", 1);
