...
public class ArrangeMRHouses : MonoBehaviour
{
        ...
        */
        ...
        for (int housesCount = 0; housesCount < houses.Count; housesCount++)
        {
            ...
            if (housesCount == chooseHouseNum)
            {
                ...
                //string groundtransfilename = Application.dataPath + "/Tayoptoutput/groundTrans_" + housesCount.ToString() + ".json";
                ...
                if (housesCount == 0)
                {
                    ...
                    for (int otherHousesCount = 0; otherHousesCount < script.otherHouses.Count; otherHousesCount++)
                    {
                        ...
                        // 이게 기존에서 얼만큼 더 이동했는지를 넣어주면 될거같은데 
                        List<float> groundTrans = groundtransDict["house_" + (otherHousesCount+1).ToString()];
