...
public class Voronoi_Test : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (changeMat == false  && arrangeHousesObj.GetComponent<Arrange_Test>().objectsAugmented == true)
        {
            for (int housesCount = 0; housesCount < houses.Count; housesCount++)
            {
                ChangeMaterial(vorShader, vorShaderWall, houses[housesCount]);
        ...
        if (initialCharactersIn == false && arrangeHousesObj.GetComponent<Arrange_Test>().characterInitialized == true)
        {
        ...
        // sharedspacePoints
        SharedSpacePoints sharedspacePoints = arrangeHousesObj.GetComponent<Arrange_Test>().sharedspacePoints;
        ...
        // update user pos 
        if (arrangeHousesObj.GetComponent<Arrange_Test>().characterInitialized == true)
        {
        ...
        List<GameObject> copyedObjectList = arrangeHousesObj.GetComponent<Arrange_Test>().copyedObjectList;
        for (int housesCount = 0; housesCount < houses.Count; housesCount++)
        {
            int mainUser;

            // 만약 object가 list에 포함되면 걔는 waive 시킨다
            ...
            else
            {
                ...
                for (int i = 0; i < rend.sharedMaterials.Length; i++)
                {
                    ...
                    if (rend.name.StartsWith("wall_"))
                    {
                        ...
                        Debug.Log("does anything falls in this?");
