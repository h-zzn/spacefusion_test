...
//#if META_AVATAR_SDK_DEFINED
    ...
    public class CustomAvatarEntity : OvrAvatarEntity, IAvatarStreamConfig
    {
        /// <summary>
        /// Event triggered when the <see cref="CustomAvatarEntity"/> is spawned.
        /// </summary>
        public static event Action<CustomAvatarEntity> OnSpawned;
        ...
        protected override void Awake()
        {
            ...
            {
                throw new InvalidOperationException("Using CustomAvatarEntity without an IAvatarBehaviour");
            }
        ...
        private void ConfigureAvatar()
        {
            ...
            if (isLocal)
            {
                ...
                // 5. LipSync 안전하게 초기화 (crash 방지)
                if (!lipSyncInitialized)
                {
                    var lipSyncInput = FindObjectOfType<OvrAvatarLipSyncContext>();
                    if (lipSyncInput != null)
                    {
                        if (!Application.isEditor || UnityEngine.XR.XRDevice.model.Contains("Quest"))
                        {
                        ...
                        {
                            Debug.LogWarning("Skipping LipSync setup in XR Simulator.");
                        }
                    }
                    else
                    {
                        Debug.LogWarning("LipSyncContext not found. Skipping initialization.");
                    }
                }
#else
